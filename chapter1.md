...
```{r}
ex() %>% check_error() 
ex() %>% check_output_expr(.,"7200",missing_msg="Da hast du dich verrechnet") %>% check_equal()
ex() %>% check_operator(.,c("*","+"),not_called_msg="Hast du auch Operatoren verwendet?") %>% check_result() %>% check_equal()
success_msg("Super!")
```
